# Docs

## Purpose
This folder contains the design, API contract, and decisions for the service.

## Contents
- architecture.md: system overview and component boundaries
- api.md: API standards and shared conventions
- design.md: project background, assumptions, and design rationale
- issues.md: setup and debugging issues log
- operations.md: remote update and operations notes
- endpoints/ping.md: endpoint-level documentation
- 原始需求/original_requirements.md: abstract requirements source of truth
- 原始需求/ai_prompt_requirements.md: prompt guidance for AI document generation
- prd.md: product requirements (simple mode)
- tdd.md: technical design (simple mode)
- roadmap.md: staged delivery plan (simple mode)
